...
5. **치환 후 문법 회귀 방지** — 용어를 전역 문자열 치환으로 끝내지 않는다. 바뀐 문장을 문장 단위로 다시 읽고 조사·서술어 호응을 고친다. 특히 `관점와`·`관점를`·`관점가`·`관점다`처럼 원래 단어의 조사가 남은 결합을 발행 차단 오류로 취급한다.
6. **병합 게이트 (필수)** — 병합도 작업이므로 검증한다(MAST '작업 검증 실패' 방지). `03_final.md`에 대해 실행하며, 이 게이트가 치환 후 조사 회귀도 검사한다:
   ```bash
