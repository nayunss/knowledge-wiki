...
OpenKnowledge는 그 사이를 메우겠다는 도구다. **마크다운 파일을 유일한 진실 원천으로 두고, 사람에게는 WYSIWYG 에디터를, 에이전트에게는 MCP 툴 20개를 같은 파일 위에 얹는다.** 별도 DB가 없고, 인덱스는 사용자가 작성한 링크·폴더·제목 자체다.
...
| 지식 엔진 | MCP 서버 — `search`·`links`·`write`·`edit` 등 20개 툴 | 에이전트 |
...
에이전트에게 관례를 가르치는 층이다. OpenKnowledge의 차별점은 **스킬이 설정 파일이 아니라 콘텐츠**라는 것 — WYSIWYG 에디터에서 다른 문서처럼 쓰고, 위키와 함께 버전 관리되고, `install` 하면 각 에디터의 스킬 디렉터리로 **심링크**된다. 복사가 아니라 심링크라서 사본 간 불일치가 없다. 한 번 고치면 모든 에이전트가 즉시 반영된다. [[하네스-엔지니어링]] 관점의 SSOT 원칙을 스킬 배포에 적용한 셈이다.
...
- 실측 시점(2026-07-11) 최신 스테이블은 v0.29.1이었고 같은 날 v0.30.0-beta.2까지 나갔다. 태그가 327개, 커밋 1,095개. **첫 공개 커밋이 2026-06-03** — 공개 저장소로서는 그때 약 5주 됐다.
- **이 속도가 얼마나 빠른지는 이 노트 자신이 증명했다.** 발행 엿새 만인 2026-07-17 기준 최신 스테이블은 **v0.32.0**(2026-07-15)이다. 그 사이 v0.30.0 → v0.30.1 → v0.31.0 → v0.32.0, 마이너 세 개가 지나갔다. 태그는 370개가 됐고, v0.33.0 베타는 하루에도 여러 번 나간다.
- 이 속도는 양날이다. 버그가 빨리 잡히지만, 당신이 의존하는 동작이 다음 마이너에서 바뀔 수 있다. 실제로 v0.29.1 다음 스테이블인 **v0.30.0(2026-07-13)이 "Breaking for consumers"를 달고 나갔다** — `preview_url` MCP 툴에서 `armPaneTarget` 파라미터가, `/api/config` 응답에서 `paneTarget` 필드가, `@inkeep/open-knowledge` 패키지에서 `scaffoldLaunchJson` 등 export가 빠졌다. 다만 공정하게 적자면 **빈도 자체는 여전히 낮다** — 최근 릴리스 100건을 훑어도 breaking을 명시한 건 이 한 건(과 그 베타)뿐이다. 그래도 MCP 툴 시그니처가 마이너에서 바뀔 수 있다는 사실은 남는다. 프로덕션 팀 위키의 유일한 편집 경로로 삼기엔 이르다.
...
- 전체 모노레포(monorepo: desktop·app·server·cli·core)가 GPL-3.0-or-later다. 도구로 **사용**하는 데는 아무 제약이 없다 — 사용자가 작성한 마크다운의 권리는 사용자에게 있다.
...
- **코드베이스 위키가 필요한 팀.** `workflow` 툴의 `discover`·`wiki` kind가 저장소를 훑어 소스 그라운디드 위키를 만든다.
...
[[llm-wiki-구조]]에서 정리했듯 이 저장소는 위키·RAG·편집 세 관점으로 동작한다. 그 셋을 OpenKnowledge와 나란히 두면 이렇게 갈린다.

| 축 | 이 위키 (Quartz + git + 하네스) | OpenKnowledge |
|---|---|---|
| 단일 원본 | `content/**.md` | 프로젝트 폴더의 `.md` |
| 편집 | 에디터 자유, 에이전트가 파일 직접 씀 | WYSIWYG + MCP 툴 20개 |
...
다만 두 가지가 충돌 지점이다. 첫째, OpenKnowledge의 자동 동기화는 **커밋을 자동으로 밀어넣는다** — 우리 파이프라인이 검증 게이트를 통과한 것만 발행한다는 원칙과 정면으로 부딪힌다. 둘째, `ok init`이 `.mcp.json`·`.claude/skills/`를 커밋 대상으로 쓴다 — 이미 하네스 설정이 있는 저장소에서는 관리 주체가 둘이 된다.

**권고:** 위키 저장소에 OpenKnowledge를 붙인다면 **`ok init --local-only` + 자동 동기화 끄기**로 시작하라. 편집·그래프 위생은 OpenKnowledge에게, 커밋·검증·발행은 기존 하네스에게. 경계가 명확할 때만 두 도구가 공존한다.
...
**후속편:** 에이전트 연동을 실제로 어떻게 쓰는지는 [[openknowledge-에이전트-활용법]]에 따로 정리했다 — `ok init`부터 코드베이스 위키 생성·그래프 위생 복구까지, 로컬 전용을 지키면서.

관련: [[openknowledge-에이전트-활용법]] · [[llm-wiki-구조]] · [[위키-설계-결정]] · [[위키-하네스]] · [[하네스-엔지니어링]] · [[컨텍스트-엔지니어링]] · [[에이전틱-엔지니어링]]
...
- OpenKnowledge 공식 문서 (로컬 클론 `docs/content/` 기준, v0.29.1):
  ...
  - MCP — 툴 명세: `docs/content/reference/mcp.mdx` (v0.32.0에서 `lint` 추가로 20개. 단 문서 산문은 여전히 "The surface is 19 tools"라 적혀 있고 헤딩만 20개다 — 2026-07-17 확인)
