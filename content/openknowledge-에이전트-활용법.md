...
이 글은 [[openknowledge]] 전편이 답한 "무엇이고 쓸 만한가" 다음 질문인 **"에이전트 연동을 실제로 어떻게 쓰는가"**에 답한다. 설치·장단점은 전편으로 넘긴다. 기준 버전은 **v0.29.1**(2026-07-11 실측, macOS 데스크톱 앱)이고, 2026-08-03 현재 최신 릴리스는 **v0.45.5**다. 아래 설정 기본값과 MCP 도구 수는 현재 버전에서 `ok config validate`, `ok --help`, 공식 MCP 문서로 다시 확인해야 한다. 본문의 v0.32.0 재확인 기록은 당시 관찰로 남긴다.
