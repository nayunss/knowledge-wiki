...
- **3단 대체 경로(fallback)** — 모델 실패·출력 변화를 견디는 구조화 출력 전략. ① 엄격 JSON 스키마 모드(모양은 강제하지만 판단은 아님) → ② Instructor + Pydantic 검증기(검증 실패 시 오류를 프롬프트에 주입해 재시도) → ③ 프롬프트 전용 + 정규화 함수. 정규화 맵을 검증기에 두어 모든 단계가 "High"를 "p1"로 바꾸게 한다. 그래도 모델들은 우선순위·감정 같은 주관적 필드에서 서로 다르게 판단하므로, 폴백은 파이프라인 전체로 시험해야 한다.
...
이 노트의 값은 "잘 되는 데모"와 "안 무너지는 시스템"을 가르는 경계를 보여준다는 데 있다. 그 경계를 넘는 실무 도구가 **프로덕션 체크리스트**다 — 관통하는 7원칙(경계마다 계약, 서버에서 거버넌스, Lusser 대비 검증, 위반 우선 평가 등)을 자기 프로토타입에 하나씩 대보는 것이다.

- **언제 쓰나**: 프로토타입이 데모를 통과해 "이제 실사용자에게 열자"는 순간. 반대로 아직 개념 검증(PoC) 단계라면 Pydantic 계약·A2A 거버넌스·샌드박스를 다 얹는 건 과설계다 — 무너져도 되는 실험엔 하드닝 세금을 내지 마라.
- **도구에 붙이기**: 기존 프로토타입 하나를 골라 4국면(기초·거버넌스·하드닝·평가) 체크리스트를 대본다. 경계마다 Pydantic 스키마를 박고, 외부 접근은 fail-closed 게이트(`governed_call()`) 하나로 모으고, 에이전트 생성 코드는 E2B 같은 격리 샌드박스에서 돌린다. 긴 작업은 블로킹 REST 대신 WebSocket/SSE로 스트리밍한다.
- **실패 패턴**: (1) Lusser의 법칙 무시 — 에이전트를 직렬로 길게 이으면 성공률이 곱으로 무너진다. 단계마다 검증을 끼워야 한다. (2) 샌드박스 없이 코드 실행 — 프롬프트 인젝션이 곧 원격 코드 실행이 된다. Monty·E2B 같은 격리 없이 모델 생성 코드를 호스트에서 돌리지 마라.

### 액션 아이템

- [ ] **오늘 30분**: 만든 프로토타입 하나를 골라 관통 7원칙에 하나씩 대보고, 지금 어기고 있는 항목을 표시한다.
- [ ] **이번 주**: 시스템 경계 한 곳(모델 출력 파싱 지점)에 Pydantic 계약을 박아, 타입 불일치를 하류가 아니라 그 자리에서 잡게 한다.
- [ ] **이번 달**: 3단 대체 경로(엄격 JSON → Instructor 검증 재시도 → 프롬프트+정규화)이나 하드닝 3기둥(체크포인팅·프루닝·서킷브레이커) 중 하나를 프로덕션 경로에 도입한다.
