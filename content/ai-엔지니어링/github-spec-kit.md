...
3. **git 주도권 — 생각보다 덜 부딪친다.** v0.12.11 기본 설치의 spec-kit은 브랜치를 만들지 않는다. `create-new-feature.sh`는 `specs/NNN-이름/` 디렉터리와 작명만 하고 git 명령을 실행하지 않는다. 따라서 **브랜치·워크트리 소유와 책임은 superpowers에 그대로 맡기면 된다.** 단, spec-kit의 git 확장을 옵트인으로 설치하는 순간 단계별 자동 커밋과 기능 브랜치 생성이 부활해 superpowers의 워크트리 흐름과 정면으로 겹친다. 그때는 **둘 중 하나에만 브랜치 책임을 맡겨라.** (0.9.x를 쓰고 있다면 이 충돌은 기본값이다.)
