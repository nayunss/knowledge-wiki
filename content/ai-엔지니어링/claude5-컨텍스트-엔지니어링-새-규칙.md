...
**이 글에서 우리 레포로 넘어오는 것은 80%라는 숫자가 아니라 삭제의 순서와 예외다.**
...
**뻔한 것 빼고 gotcha에 토큰 쓰기.** 원문의 `CLAUDE.md` 지침은 명확하다. 레포 용도는 짧게 쓰고, 토큰 대부분을 코드베이스의 함정에 쓰고, 파일시스템이나 레포만 봐도 아는 것은 적지 말라는 것이다. 이 규칙이 좋은 이유는 삭제 판단이 쉽기 때문이다. 에이전트가 도구로 1초 만에 확인할 수 있는 사실이면 텍스트로 중복해 적을 이유가 약하다.
