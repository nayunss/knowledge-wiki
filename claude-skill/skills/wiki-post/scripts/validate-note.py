...
def main():
    ...
    # 코드펜스·인라인코드 안은 "설명"이지 "잔여물"이 아니다 (메타 노트가 마커·깨진 패턴을 인용한다)
    ...
    if "(검증 필요)" in prose:
        fails.append(f"'(검증 필요)' 마커 {prose.count('(검증 필요)')}건 잔존 — 검증 후 제거/완화 필요")

    # 2.5) 마크다운 렌더 검증 (발행물에서 리터럴 ** 노출 사고 재발 방지)
    for mm in re.finditer(r"[)\]]\*\*[가-힣]", prose):
        fails.append(f"깨진 강조(닫는 ** 앞 괄호+뒤 한글): …{prose[max(0,mm.start()-15):mm.end()+5]}…")
    for mm in re.finditer(r"[\"\u201d\u2019']\*\*[가-힣]", prose):
        warns.append(f"강조 인접 따옴표 — 렌더 확인 필요: …{prose[max(0,mm.start()-15):mm.end()+5]}…")
    for mm in re.finditer(r"~(?=[0-9])", prose):
        warns.append(f"물결 표기 발견(근사→약 N, 범위→N–M 권장, GFM 취소선 위험): …{prose[max(0,mm.start()-10):mm.end()+8]}…")
